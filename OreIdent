...
local function newLayer()
 ...
 if false and not turtle.detect() then
  turtle.turnLeft()
  turtle.turnLeft()
...
x = 0
while x < length do
 while not tryGo("f") do
  lprint("going up")
  while not tryGo("u") do
   lprint("going back")
   if not go("b") then
    dig("b")
    ok, reason = go("b")
    if not ok then
     lerror("Failed to go back, because: "..tostring(reason))
    end
   end
   x = x - 1
  end
 ...
 if y > y0 then
  lprint("Going from: x = "..x..", y = "..y)
 end
 while y > y0 do
  lprint("going down")
  while not tryGo("d") do
   lprint("going forward")
   while not tryGo("f") do
    lprint("going up")
    while not tryGo("u") do
     lprint("going back")
